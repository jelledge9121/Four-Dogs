...
export default function CheckInForm() {
    ...
    return (
      <div className="fd-checkin-success">
        ...
        {successPayload.bonuses_earned.length > 0 ? (
          <p className="fd-note-info">Bonus: {successPayload.bonuses_earned.join(', ')}</p>
        ) : null}

        {successPayload.next_reward ? (
          <p className="fd-note-info">
  ...
  return (
    <form onSubmit={handleSubmit} className="fd-checkin-form">
      <div>
      ...
      {eventsLoadError ? <p className="fd-note-warn">{eventsLoadError}</p> : null}
      {status === 'error' ? <p className="fd-note-warn">{errorMessage}</p> : null}

      <button type="submit" disabled={!canSubmit} className="fd-primary-button">
